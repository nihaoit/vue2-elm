...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      [this.shopDetailData, this.menuList] = [
        await shopDetails(this.shopId, this.latitude, this.longitude),
        await foodMenu(this.shopId)
      ];
      //评论列表
    ...
    //控制购物列表是否显示
    toggleCartList() {
      console.log('toggleCartList');
      // 下面这行代码的理解
      // 如果购物车列表中有食品，那么就执行返回第一部分，第一部分是个赋值语句
      // 否则就返回第二部分，相当于什么也不做
